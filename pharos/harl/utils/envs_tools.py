    ...
    def get_env_fn(rank):
        def init_env():
            if env_name == "smac":
                ...
                env = LAGEnv(env_args)
            elif env_name == "pharos":
                from harl.envs.pharos.pharos_env import PharosEnv
                env = PharosEnv(env_args)
            elif env_name == "pharos_discrete":
                from harl.envs.pharos_discrete.pharos_env import PharosDiscreteEnv
                env = PharosDiscreteEnv(env_args)
            else:
    ...
    def get_env_fn(rank):
        def init_env():
            ...
            # HINT: Add here
                ...
                from harl.envs.pharos_discrete.pharos_env import PharosDiscreteEnv as PharosEnvDiscrete
                env = PharosEnvDiscrete(env_args)
            else:
                print("Can not support the " + env_name + "environment.")
                raise NotImplementedError
