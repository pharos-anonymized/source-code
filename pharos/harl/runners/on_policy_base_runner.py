...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if self.algo_args["render"]["use_render"]:  # make envs for rendering
            ...
            if self.eval_envs is None:
                warn("Eval envs are None, please check your config.")
        self.num_agents = get_num_agents(args["env"], env_args, self.envs)

        print("[INFO]share_observation_space: ", self.envs.share_observation_space)
        print("[INFO]observation_space: ", self.envs.observation_space)
        print("[INFO]action_space: ", self.envs.action_space)
        ...
        if self.algo_args["render"]["use_render"] is False:  # train, not render
            ...
            # )
            if args["env"] == "pharos" or args["env"] == "pharos_discrete":
                # HINT: 由于pharos需要保存json渲染数据, 必须传入envs耦合
                self.logger = LOGGER_REGISTRY[args["env"]](
                    args,
                    algo_args,
                    env_args,
                ...
                print(f"For Pharos env, the logger is set to {self.logger}")
            else:
                self.logger = LOGGER_REGISTRY[args["env"]](
                    args,
                    algo_args,
    ...
    def run(self):
        ...
        for episode in tqdm(range(1, episodes + 1)):
            ...
            if self.algo_args["train"]["use_cosine_lr_decay"]:
                # cosine decay of learning rate
                if self.share_param:
                    self.actor[0].lr_decay_cosine(episode, episodes)
                else:
                    for agent_id in range(self.num_agents):
                        self.actor[agent_id].lr_decay_cosine(episode, episodes)
                self.critic.lr_decay_cosine(episode, episodes)
    ...
    def insert(self, data):
        ...
        # TODO: 适配MultiDiscrete的AvailableActions
        (
            obs,  # (n_threads, n_agents, obs_dim)
            share_obs,  # (n_threads, n_agents, share_obs_dim)
            rewards,  # (n_threads, n_agents, 1)
        ...
        while True:
            ...
            for eval_i in range(self.algo_args["eval"]["n_eval_rollout_threads"]):
                if eval_dones_env[eval_i]:
                    eval_episode += 1
                    print(f"[INFO] Evaluation episode {eval_episode} done.")
